...
import { getPatientTrajectories, getScoreDistribution, getScoreTrends, getTrajectoryWorkflowData, computeSectionStatus, hasAnyMissingRequired } from "@/lib/admin-data-utils"
...
export function AdminPanel() {
  ...
  // Section status review (cognition / tcm / sensory), admin remarks & reminders
  const SECTION_KEYS = ["cognition", "tcm", "sensory"] as const
  ...
  const [exemptFlags, setExemptFlags] = useState<Record<string, boolean>>({})
  ...
  useEffect(() => {
    ...
    // Section statuses + admin remarks/reminders for the selected patient
    if (!selectedUser) {
      setSectionStatuses(null)
      setActiveReminders([])
      return
    }
    setSectionStatuses(computeSectionStatus(selectedUser, assessments, sensoryAssessments, tcmAssessments))
    setRemarkSaveState({})
    ;(async () => {
      try {
        ...
        ;(notes || []).forEach((n: any) => {
          remarks[n.section] = n.note ?? ""
          exempt[n.section] = !!n.exempt
        })
        setAdminRemarks(remarks)
        setExemptFlags(exempt)
        ...
        console.warn("[v0] Failed to load admin notes/reminders:", err)
      }
    })()
  }, [selectedUser, doctorReviews, medicalReports, tcmAssessments, assessments, sensoryAssessments])
      ...
      const { error } = await supabase.from("admin_notes").upsert(
        {
          ...
          section,
          note: text,
          exempt: !!exemptFlags[section],
          updated_at: new Date().toISOString(),
        },
        { onConflict: "user_id,section" },
      )
      setRemarkSaveState((prev) => ({ ...prev, [section]: error ? "pending" : "saved" }))
    } catch (err) {
  ...
  const toggleExempt = async (section: string) => {
    if (!selectedUser) return
    const newValue = !exemptFlags[section]
    setExemptFlags((prev) => ({ ...prev, [section]: newValue }))
    try {
  ...
  const createMissingExamReminders = async () => {
    ...
    const missingSections = SECTION_KEYS.filter(
      (k) => sectionStatuses[k] && sectionStatuses[k].status !== "completed" && !exemptFlags[k],
    )
            ...
            <CardContent>
              {selectedUser ? (
                <div className="space-y-4">
                  {/* Section status review: Cognition / TCM / Sensory */}
                  {sectionStatuses && (
                    <div className="space-y-3">
                      {hasAnyMissingRequired(sectionStatuses) && (
                        <div className="p-3 rounded-lg bg-rose-50 border border-rose-200 flex items-center justify-between flex-wrap gap-2">
                          <div>
                            <p className="font-semibold text-rose-700">
                              {localizeText("Missing required exams", { zh: "缺少必需检查", yue: "缺少必需檢查", fr: "Examens requis manquants" })}
                            </p>
                            <p className="text-sm text-rose-600">
                              {localizeText("Some required sections are incomplete for this patient.", { zh: "该患者部分必需项目尚未完成。", yue: "呢位患者部分必需項目未完成。", fr: "Certaines sections requises sont incompletes." })}
                            </p>
                          </div>
                          <Button size="sm" onClick={createMissingExamReminders}>
                            {localizeText("Create reminders", { zh: "创建提醒", yue: "建立提醒", fr: "Creer des rappels" })}
                          </Button>
                        </div>
                      )}
                      ...
                      {activeReminders.length > 0 && (
                        <div className="p-3 rounded-lg bg-amber-50 border border-amber-200 space-y-2">
                          <p className="font-semibold text-amber-800">
                            {localizeText("Pending follow-up reminders", { zh: "待处理跟进提醒", yue: "待處理跟進提醒", fr: "Rappels de suivi en attente" })}
                          </p>
                          {activeReminders.map((r) => (
                            <div key={r.id} className="flex items-center justify-between text-sm gap-2">
                              ...
                              <Button size="sm" variant="outline" onClick={() => resolveReminder(r.id)}>
                                {localizeText("Mark done", { zh: "标记完成", yue: "標記完成", fr: "Marquer fait" })}
                              </Button>
                            </div>
                          ))}
                        </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        {SECTION_KEYS.map((section) => {
                          const s = sectionStatuses[section]
                          if (!s) return null
                          const statusColor =
                            s.status === "completed"
                              ? "border-emerald-200 bg-emerald-50 text-emerald-700"
                              : s.status === "in_progress"
                                ? "border-amber-200 bg-amber-50 text-amber-700"
                                : "border-rose-200 bg-rose-50 text-rose-700"
                          const sectionLabel = localizeText(
                            section === "cognition" ? "Cognition" : section === "tcm" ? "TCM" : "Sensory",
                            section === "cognition"
                            ...
                            <div key={section} className="border rounded-lg p-3 space-y-2">
                              <div className="flex items-center justify-between">
                                ...
                                <Badge variant="outline" className={statusColor}>
                                  {s.status === "completed"
                                    ? localizeText("Completed", { zh: "已完成", yue: "已完成", fr: "Termine" })
                                    : s.status === "in_progress"
                                      ? localizeText("In progress", { zh: "进行中", yue: "進行中", fr: "En cours" })
                                      : localizeText("Missing", { zh: "缺失", yue: "缺失", fr: "Manquant" })}
                                </Badge>
                              </div>
                              {s.missingItems && s.missingItems.length > 0 && (
                                <p className="text-xs text-rose-600">
                                  {localizeText("Missing", { zh: "缺少", yue: "缺少", fr: "Manquant" })}: {s.missingItems.join(", ")}
                                </p>
                              )}
                              <textarea
                                id={`remark-${section}`}
                                className="w-full text-sm border rounded p-2 min-h-[60px]"
                                placeholder={localizeText("Admin remark…", { zh: "管理员备注…", yue: "管理員備註…", fr: "Remarque admin…" })}
                                value={adminRemarks[section] || ""}
                                onChange={(e) => setAdminRemarks((prev) => ({ ...prev, [section]: e.target.value }))}
                              />
                              <div className="flex items-center gap-2 flex-wrap">
                                <Button size="sm" onClick={() => saveAdminRemark(section, adminRemarks[section] || "")}>
                                  {localizeText("Save", { zh: "保存", yue: "儲存", fr: "Enregistrer" })}
                                </Button>
                                <Button
                                  size="sm"
                                  variant={exemptFlags[section] ? "default" : "outline"}
                                  onClick={() => toggleExempt(section)}
                                >
                                  {exemptFlags[section]
                                    ? localizeText("Exempt ✓", { zh: "已豁免 ✓", yue: "已豁免 ✓", fr: "Exempte ✓" })
                                    : localizeText("Mark Exempt", { zh: "标记豁免", yue: "標記豁免", fr: "Marquer exempte" })}
                                </Button>
                                {remarkSaveState[section] === "saving" && <span className="text-xs text-slate-500">{localizeText("Saving…", { zh: "保存中…", yue: "儲存中…", fr: "Enregistrement…" })}</span>}
                                {remarkSaveState[section] === "saved" && <span className="text-xs text-emerald-600">{localizeText("Saved", { zh: "已保存", yue: "已儲存", fr: "Enregistre" })}</span>}
                                {remarkSaveState[section] === "pending" && <span className="text-xs text-amber-600">{localizeText("Saved locally", { zh: "已本地保存", yue: "已本地儲存", fr: "Enregistre localement" })}</span>}
                              </div>
                            </div>
                          )
                        })}
                      ...
                      <div className="mt-6 rounded-2xl border border-sky-200 bg-[linear-gradient(135deg,rgba(240,249,255,0.96),rgba(255,255,255,0.98),rgba(238,242,255,0.94))] p-5 shadow-sm">
                        ...
                        <div className="mt-4 space-y-2">
                          ...
                          <AssessmentTextarea
                            ...
                            placeholder={localizeText("Food strategy and restrictions", { zh: "饮食策略���禁忌", yue: "飲食策略與禁忌", fr: "Strategie alimentaire et restrictions" })}
                            rows={2}
                          />
                        </div>
