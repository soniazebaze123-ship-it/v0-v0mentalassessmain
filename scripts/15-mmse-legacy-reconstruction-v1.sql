-- ============================================================================
-- MIGRATION: MMSE legacy reconstruction to 30-point equivalent (versioned)
-- File: 15-mmse-legacy-reconstruction-v1.sql
-- Purpose:
--   1) Preserve historical legacy MMSE scores (do not overwrite provenance)
--   2) Add score-version metadata columns for auditability
--   3) Reconstruct legacy MMSE records into a 30-point model for continuity
--   4) Keep a full audit trail for rollback and reproducibility
--
-- Notes:
--   - This migration updates ONLY MMSE rows with reconstruction_applied = false.
--   - Legacy values are preserved in dedicated columns and audit table.
--   - Uses reconstructed framework agreed for legacy cohort handling.
-- ============================================================================
...
-- Backfill max_score for legacy rows if not set.
...
-- --------------------------------------------------------------------------
-- 2) Audit table for deterministic rollback
-- --------------------------------------------------------------------------
...
-- Safe integer parsing helper for historical JSON payloads.
...
-- --------------------------------------------------------------------------
-- 3) Reconstruct legacy MMSE rows and persist provenance
-- --------------------------------------------------------------------------
WITH params AS (
  SELECT
    'MMSE_RECON_V1_2026_05_22'::TEXT AS run_label,
    'MMSE_v1_reconstructed_30'::TEXT AS scoring_version,
    'MentalAssess MMSE Legacy Cohort Recalculation v1'::TEXT AS scoring_framework
),
source AS (
  SELECT
    a.id,
...
calc AS (
  SELECT
    ...
    -- Orientation (split to time/place for reconstructed framework)
    LEAST(5, GREATEST(0,
      COALESCE(
    ...
    -- Naming tiering as agreed for legacy reconstruction
    CASE
...
prepared AS (
  SELECT
    ...
    -- Preserve original section data and append reconstruction metadata.
...
-- --------------------------------------------------------------------------
SELECT
  COUNT(*) FILTER (WHERE UPPER(type) = 'MMSE') AS mmse_total_rows,
  COUNT(*) FILTER (WHERE UPPER(type) = 'MMSE' AND reconstruction_applied = true) AS mmse_reconstructed_rows,
  COUNT(*) FILTER (WHERE UPPER(type) = 'MMSE' AND score_legacy IS NOT NULL) AS mmse_with_legacy_backup,
  COUNT(*) FILTER (WHERE UPPER(type) = 'MMSE' AND max_score = 30) AS mmse_max_30_rows
